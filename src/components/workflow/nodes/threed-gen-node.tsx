...
export function ThreedGenNode({ node }: ThreedGenNodeProps) {
  const modelOutput = node.outputs['output_1_model3d']?.value as string | undefined
  const hasInputImage = !!(node.outputs['output_1_model3d'] || node.ports.find(
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-2 rounded-md bg-white/5 px-2 py-1.5 border border-white/10">
          ...
          <span className="text-[10px] text-slate-400">
            {hasInputImage ? 'Connect an image to convert to 3D' : 'Image input required'}
          </span>
        </div>
      </div>
      ...
      {node.status === 'completed' && modelOutput && (
        <div className="flex flex-col gap-1">
          <Label className="text-[10px] uppercase tracking-wider text-slate-400">
            3D Model
          </Label>
          <div className="relative overflow-hidden rounded-md border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 to-transparent aspect-square flex items-center justify-center">
            <div className="flex flex-col items-center gap-1.5">
              <Box className="h-8 w-8 text-cyan-400/80" />
              <span className="text-[10px] text-cyan-400/80">3D Model Ready</span>
              {modelOutput && (
                <span className="text-[9px] text-slate-500">
                  {modelOutput.startsWith('data:model') ? 'GLB Model' : '3D Asset'}
                </span>
              )}
            </div>
            {/* Decorative grid */}
